...
func newElasticsearchConfigFromSpec(c k8s.Client, as *apmv1.ApmEsAssociation) (*settings.CanonicalConfig, error) {
	...
	if as.AssociationConf().GetCACertProvided() {
		tmpOutputCfg["output.elasticsearch.ssl.certificate_authorities"] = []string{filepath.Join(certificatesDir(as.AssociatedServiceType()), certificates.CAFileName)}
	}

	return settings.MustCanonicalConfig(tmpOutputCfg), nil
}

func newKibanaConfigFromSpec(c k8s.Client, as *apmv1.ApmKibanaAssociation) (*settings.CanonicalConfig, error) {
	if !as.AssociationConf().IsConfigured() {
		return settings.NewCanonicalConfig(), nil
	}
	...
	tmpOutputCfg := map[string]interface{}{
		...
		"apm-server.kibana.host":     as.AssociationConf().GetURL(),
		"apm-server.kibana.username": username,
		"apm-server.kibana.password": password,
	}
	if as.AssociationConf().GetCACertProvided() {
		tmpOutputCfg["apm-server.kibana.ssl.certificate_authorities"] = []string{filepath.Join(certificatesDir(as.AssociatedServiceType()), certificates.CAFileName)}
	}
