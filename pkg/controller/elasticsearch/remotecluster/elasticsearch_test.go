...
func Test_getCurrentRemoteClusters(t *testing.T) {
	...
	tests := []struct {
		name    string
		args    args
		want    map[string]types.NamespacedName
		wantErr bool
	}{
		/*{
		...
		},*/
		{
			...
			args: args{es: esv1.Elasticsearch{
				ObjectMeta: metav1.ObjectMeta{
					Name:        "ns1",
					Namespace:   "es1",
					Annotations: map[string]string{"elasticsearch.k8s.elastic.co/remote-clusters": `[{"Namespace":"ns1","Name":"es2"},{"Namespace":"ns2","Name":"es1"}]`},
				},
			}},
			want: map[string]types.NamespacedName{
				"ns1-es2": {
