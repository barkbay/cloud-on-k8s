...
func CustomTransportCertsWatchKey(es types.NamespacedName) string {
	return escommon.StatefulNamer.Suffix(es.Name, "custom-transport-certs")
}
...
func ReconcileOrRetrieveCA(
	...
	if err := certificates.ReconcileCustomCertWatch(
		...
		esNSN,
		es.GetTransport().TLS.Certificate,
	); err != nil {
		return nil, err
	}

	customCASecret, err := certificates.GetSecretFromRef(driver.K8sClient(), esNSN, es.GetTransport().TLS.Certificate)
	if err != nil {
	...
	err = driver.K8sClient().Delete(ctx, &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      certificates.CAInternalSecretName(namer, esNSN.Name, certificates.TransportCAType),
