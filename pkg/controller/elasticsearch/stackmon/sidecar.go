...
func Metricbeat(ctx context.Context, client k8s.Client, es MonitoredElasticsearch, meta metadata.Metadata) (stackmon.BeatSidecar, error) {
	...
	// Get the appropriate namer based on cluster type
	namer := escommon.StatefulNamer
	if es.IsStateless() {
		namer = escommon.StatelessNamer
	}

	caVolume, err := stackmon.CAVolume(client, k8s.ExtractNamespacedName(es), namer, commonv1.EsMonitoringAssociationType, es.GetHTTP().TLS.Enabled())
	if err != nil {
		return stackmon.BeatSidecar{}, err
	}

	input := stackmon.TemplateParams{
		URL:      fmt.Sprintf("%s://localhost:%d", es.GetHTTP().Protocol(), network.HTTPPort),
		Username: username,
		Password: password,
		IsSSL:    es.GetHTTP().TLS.Enabled(),
		CAVolume: caVolume,
	}
...
func Filebeat(ctx context.Context, client k8s.Client, es MonitoredElasticsearch, meta metadata.Metadata) (stackmon.BeatSidecar, error) {
	fileBeat, err := stackmon.NewFileBeatSidecar(ctx, client, es, es.GetVersion(), filebeatConfig, nil, meta)
	if err != nil {
		return stackmon.BeatSidecar{}, err
	}
	ver, err := version.Parse(es.GetVersion())
...
func WithMonitoring(ctx context.Context, client k8s.Client, builder *defaults.PodTemplateBuilder, es MonitoredElasticsearch, meta metadata.Metadata) (*defaults.PodTemplateBuilder, error) {
