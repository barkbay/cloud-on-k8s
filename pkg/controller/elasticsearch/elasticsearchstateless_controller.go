...
const statelessControllerName = "elasticsearchstateless-controller"
...
func newStatelessReconciler(mgr manager.Manager, params operator.Parameters) *ReconcileElasticsearchStateless {
	...
	return &ReconcileElasticsearchStateless{
		...
		expectations:   expectations.NewClustersExpectations(client, &corev1.Pod{}),
...
func addStatelessWatches(mgr manager.Manager, c controller.Controller, r *ReconcileElasticsearchStateless) error {
	...
	// Watch pods belonging to ESS clusters
...
func (r *ReconcileElasticsearchStateless) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	state := newStatelessState(ess)

	results := r.internalReconcileStateless(ctx, ess, state)

	// Update orchestration related annotations
	if err := r.annotateStatelessResource(ctx, ess, state); err != nil {
	...
	if isReconciled, message := results.IsReconciled(); !isReconciled {
		state.UpdateWithPhase(escommon.ElasticsearchApplyingChangesPhase)
		state.ReportCondition(escommon.ReconciliationComplete, corev1.ConditionFalse, message)
	} else {
		state.UpdateWithPhase(escommon.ElasticsearchReadyPhase)
	}

	// Last step of the reconciliation loop is always to update the ElasticsearchStateless resource status.
	err = r.updateStatelessStatus(ctx, ess, state)
	if err != nil {
		if apierrors.IsConflict(err) {
...
func (r *ReconcileElasticsearchStateless) fetchElasticsearchStatelessWithAssociations(ctx context.Context, request reconcile.Request, ess *essv1alpha1.ElasticsearchStateless) (bool, error) {
	...
	if err := r.Client.Get(ctx, request.NamespacedName, ess); err != nil {
		if apierrors.IsNotFound(err) {
			// Object not found, cleanup in-memory state.
			return true, r.onStatelessDelete(ctx,
...
func (r *ReconcileElasticsearchStateless) internalReconcileStateless(
	ctx context.Context,
	ess essv1alpha1.ElasticsearchStateless,
	reconcileState *statelessState,
) *reconciler.Results {
	results := reconciler.NewResult(ctx)
	log := log.FromContext(ctx)
	if ess.IsMarkedForDeletion() {
		// resource will be deleted, nothing to reconcile
		return results.WithError(r.onStatelessDelete(ctx, k8s.ExtractNamespacedName(&ess)))
	}

	// TODO: Add validation for ElasticsearchStateless
	// For now we skip validation as it's not yet implemented
	...
	log.V(1).Info("Reconciling ElasticsearchStateless", "namespace", ess.Namespace, "name", ess.Name, "version", ver)

	return stateless.NewDriver(stateless.Parameters{
		Parameters: shared.Parameters{
			...
			ReconcileState:     reconcileState.esReconcileState,
		},
		ES: ess,
	}).Reconcile(ctx)
}

func (r *ReconcileElasticsearchStateless) updateStatelessStatus(
	ctx context.Context,
	ess essv1alpha1.ElasticsearchStateless,
	reconcileState *statelessState,
) error {
	...
	evts, cluster := reconcileState.Apply()
...
// annotateStatelessResource adds the orchestration hints annotation to the ElasticsearchStateless resource.
func (r *ReconcileElasticsearchStateless) annotateStatelessResource(
	ctx context.Context,
	ess essv1alpha1.ElasticsearchStateless,
	reconcileState *statelessState,
) error {
...
// onStatelessDelete garbage collect resources when an ElasticsearchStateless cluster is deleted
func (r *ReconcileElasticsearchStateless) onStatelessDelete(ctx context.Context, ess types.NamespacedName) error {
