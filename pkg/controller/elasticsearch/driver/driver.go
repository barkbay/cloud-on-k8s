...
// Package driver provides the Elasticsearch driver API and shared types.
package driver
...
// Parameters contain parameters for the driver.
...
type Parameters struct {
	...
	// ReconcileState holds the accumulated state during the reconcile loop.
	ReconcileState *reconcile.State

	// Observers observe Elasticsearch clusters state.
	Observers *observer.Manager

	// DynamicWatches are handles to currently registered dynamic watches.
	DynamicWatches watches.DynamicWatches

	// Expectations control expectations set on resources in the cache, in order to
