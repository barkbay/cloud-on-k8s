...
func TestIsImmutableName(t *testing.T) {
	tests := []struct {
		name string
		arg  string
		want bool
		...
		{
			name: "valid immutable name",
			arg:  "my-config-a1b2c3d4",
			want: true,
		},
		{
			name: "valid with longer base",
			arg:  "elasticsearch-es-index-es-config-deadbeef",
			want: true,
		},
		{
			name: "too short hash",
			arg:  "my-config-a1b2c3",
			want: false,
		},
		{
			name: "no hash suffix",
