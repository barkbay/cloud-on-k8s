...
func deleteAllRemoteCa(ctx context.Context, r *baseRemoteClustersReconciler, es types.NamespacedName, isStateless bool) (reconcile.Result, error) {
	...
	associatedCAs, err := getAssociatedRemoteCAs(ctx, r.Client, es, isStateless)
	if err != nil {
		return reconcile.Result{}, err
	}
	results := &reconciler.Results{}
	for remoteCluster := range associatedCAs {
		if err := deleteCertificateAuthorities(ctx, r, es, remoteCluster, isStateless); err != nil {
...
func doReconcile(
	...
	associatedRemoteCAs, err := getAssociatedRemoteCAs(ctx, r.Client, remoteServerKey, remoteServer.IsStateless())
	if err != nil {
		return reconcile.Result{}, err
	}
	...
	for remoteClientKey, remoteClusterRefs := range expectedRemoteClients {
		...
		delete(associatedRemoteCAs, remoteClientKey.NamespacedName())
	...
	for toDelete := range associatedRemoteCAs {
		...
		results.WithError(deleteCertificateAuthorities(ctx, r, remoteServerKey, toDelete, remoteServer.IsStateless()))
	}
...
func getAssociatedRemoteCAs(
	...
	if err := c.List(ctx,
		...
		remoteca.Labels(es.Name, isStateless),
	); err != nil {
		return nil, err
	}
	...
	// Check both stateful and stateless cluster labels.
	if err := c.List(ctx,
		&remoteCAList,
	...
	for _, remoteCA := range remoteCAList.Items {
		// Try stateful label first, then stateless
		remoteEs := remoteCA.Labels[label.ClusterNameLabelName]
		if remoteEs == "" {
			remoteEs = remoteCA.Labels[label.StatelessClusterNameLabelName]
		}
		if remoteEs != "" {
			currentRemoteClusters[types.NamespacedName{
