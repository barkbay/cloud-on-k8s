...
type pendingChange struct {
	remoteClusterName, remoteClusterNamespace, remoteClusterKind, alias string
	key                                                                 key
}
...
func (pc *pendingChanges) AddKey(remoteClusterName, remoteClusterNamespace, remoteClusterKind, alias, keyID, encodedKeyValue string) {
	if pc == nil {
		return
	}
	pc.mu.Lock()
	...
	pc.changes[alias] = pendingChange{
		...
		remoteClusterKind:      remoteClusterKind,
		alias:                  alias,
		key: key{
			keyID:        keyID,
...
func (pc *pendingChanges) Get() []pendingChange {
	...
	for alias, change := range pc.changes {
		pendingChange := pendingChange{
			...
			remoteClusterKind:      change.remoteClusterKind,
			alias:                  alias,
			key: key{
