...
| *`count`* __integer__ | Count of Elasticsearch nodes to deploy. Optional if autoscaling is enabled, in that case the autoscaling controller automatically sets the minimum number of nodes specified in the autoscaling policy.
